...
class OwedItem(models.Model):
    ...
    def _get_owed_item(cls, username_1, username_2, item_type):
        ...
        """
        try:
            item = cls.objects.get(owed_username=username_1.lower(),
                                   ower_username=username_2.lower(),
                                   item_type=item_type)
        except OwedItem.DoesNotExist:
            try:
                item = cls.objects.get(owed_username=username_2.lower(),
                                       ower_username=username_1.lower(),
                                       item_type=item_type)
            except OwedItem.DoesNotExist:
    ...
    def ensure_owed_item(cls, owed_username,
        ...
        try:
            item = cls._get_owed_item(owed_username, ower_username, item_type)

            if item.ower_username.lower() == ower_username.lower():
